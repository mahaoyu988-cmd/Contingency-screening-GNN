...
# Data file paths
# ============================================================
train_book = r"C:\Users\haoyu.ma\OneDrive - Washington State University (email.wsu.edu)\Research\Datasetgeneration\case118_contingency_pf_dataset_8scen_8cont.xlsx"
...
def split_features_targets_with_genbus(main_df, dispatch_df, n_bus,
    ...
    for i in range(len(main_np)):
        ...
        for g in range(ng):
            ...
            Pgen = disp_np[i, 2 * g + 2]   # sample_id, contingency_index, then Pgen_1
            Qgen = disp_np[i, 2 * g + 3]
            Pgen_bus[bus_idx] += Pgen
            Qgen_bus[bus_idx] += Qgen

        for n in range(n_bus):
            Pload = main_np[i, 4 * n + 2]   # sample_id, contingency_index, then Pload_1
...
def build_dynamic_dataset(x_tensor, y_tensor, meta_df, branch_df,
    ...
    for i in range(x_tensor.size(0)):
        ...
        cont_idx = cont_idx_list[i] - 1   # 1-based -> 0-based row index
...
# Must match MATLAB export generator order
gen_bus_vec = [
...
train_list = build_dynamic_dataset(
    x_train, y_train, meta_train_df, branch_df,
    from_bus_orig, to_bus_orig, edge_attr_orig
)

val_list = build_dynamic_dataset(
    x_val, y_val, meta_val_df, branch_df,
    from_bus_orig, to_bus_orig, edge_attr_orig
)

test_list = build_dynamic_dataset(
    x_test, y_test, test_meta_df, branch_df,
    from_bus_orig, to_bus_orig, edge_attr_orig
)
...
# ============================================================
# Testing: combined + separate V / delta metrics + max abs errors
# ============================================================
