...
%% ============================================================
% Random testing scenarios from seasonal convex combinations
%% ============================================================

nScen = 50;   % choose how many testing scenarios you want

scenario_defs = cell(nScen, 4);
scenario_weight_rows = cell(nScen, 13);

for s = 1:nScen
    % -------------------------
    % Random weights for load
    % -------------------------
    wL = rand(1,4);
    wL = wL / sum(wL);

    % -------------------------
    % Random weights for wind
    % -------------------------
    wW = rand(1,4);
    wW = wW / sum(wW);

    % -------------------------
    % Random weights for solar
    % -------------------------
    wS = rand(1,4);
    wS = wS / sum(wS);
    ...
    % -------------------------
    load_curve_s  = LoadMat * wL';
    wind_curve_s  = WindMat * wW';
    solar_curve_s = PVMat * wS';

    scen_name = sprintf('RandScen_%03d', s);

    scenario_defs{s,1} = scen_name;
    scenario_defs{s,2} = load_curve_s;
    scenario_defs{s,3} = wind_curve_s;
    scenario_defs{s,4} = solar_curve_s;

    scenario_weight_rows(s,:) = { ...
        scen_name, ...
        wL(1), wL(2), wL(3), wL(4), ...
        wW(1), wW(2), wW(3), wW(4), ...
        wS(1), wS(2), wS(3), wS(4)};
...
scenario_weights = cell2table(scenario_weight_rows, ...
    'VariableNames', { ...
    'scenario_name', ...
    'Load_w_summer', 'Load_w_winter', 'Load_w_spring', 'Load_w_fall', ...
    'Wind_w_summer', 'Wind_w_winter', 'Wind_w_spring', 'Wind_w_fall', ...
    'Solar_w_summer', 'Solar_w_winter', 'Solar_w_spring', 'Solar_w_fall'});

%% ============================================================
% Export to Excel
%% ============================================================
outfile = 'case118_contingency_pf_random_test_dataset.xlsx';
...
%% ============================================================
% Plot max/min and 5 random curves for Load, Wind, Solar
%% ============================================================
hours = (1:24)';

% choose 5 random scenarios from the generated random scenarios
rng(1);   % optional, for reproducibility
idx_plot = randperm(nScen, 5);

% collect the selected random curves
Load_rand_plot  = zeros(24, 5);
Wind_rand_plot  = zeros(24, 5);
Solar_rand_plot = zeros(24, 5);

for k = 1:5
    s = idx_plot(k);
    Load_rand_plot(:,k)  = scenario_defs{s,2};
    Wind_rand_plot(:,k)  = scenario_defs{s,3};
    Solar_rand_plot(:,k) = scenario_defs{s,4};
end
...
plot(hours, Lmax, 'r-', 'LineWidth', 2);
plot(hours, Lmin, 'b-', 'LineWidth', 2);

for k = 1:5
    plot(hours, Load_rand_plot(:,k), '--', 'LineWidth', 1.5);
end

xlabel('Hour');
ylabel('Load Multiplier');
title('Load Curves: Max / Min / 5 Random Scenarios');
legend_entries = [{'Lmax','Lmin'}, ...
    arrayfun(@(x) sprintf('Random %d', x), 1:5, 'UniformOutput', false)];
legend(legend_entries, 'Location', 'best');
xlim([1 24]);
xticks(1:24);

%% ------------------------------------------------------------
% Figure 2: Wind curves
%% ------------------------------------------------------------
figure;
hold on; grid on; box on;

plot(hours, Wmax, 'r-', 'LineWidth', 2);
plot(hours, Wmin, 'b-', 'LineWidth', 2);

for k = 1:5
    plot(hours, Wind_rand_plot(:,k), '--', 'LineWidth', 1.5);
end

xlabel('Hour');
ylabel('Wind Capacity Factor');
title('Wind Curves: Max / Min / 5 Random Scenarios');
legend_entries = [{'Wmax','Wmin'}, ...
    arrayfun(@(x) sprintf('Random %d', x), 1:5, 'UniformOutput', false)];
legend(legend_entries, 'Location', 'best');
xlim([1 24]);
xticks(1:24);
...
%% ------------------------------------------------------------
% Figure 3: Solar curves
%% ------------------------------------------------------------
figure;
hold on; grid on; box on;

plot(hours, PVmax, 'r-', 'LineWidth', 2);
plot(hours, PVmin, 'b-', 'LineWidth', 2);

for k = 1:5
    plot(hours, Solar_rand_plot(:,k), '--', 'LineWidth', 1.5);
end

xlabel('Hour');
ylabel('Solar Capacity Factor');
title('Solar Curves: Max / Min / 5 Random Scenarios');
legend_entries = [{'PVmax','PVmin'}, ...
    arrayfun(@(x) sprintf('Random %d', x), 1:5, 'UniformOutput', false)];
legend(legend_entries, 'Location', 'best');
xlim([1 24]);
xticks(1:24);
...
%% ------------------------------------------------------------
% Print which random scenarios were selected
%% ------------------------------------------------------------
fprintf('Selected random scenarios for plotting:\n');
disp(idx_plot);